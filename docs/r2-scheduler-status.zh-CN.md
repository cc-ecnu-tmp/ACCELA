...
2026-08-16 的 VisionFive 2 U74 归档覆盖 100 个公开 functional 用例：94 个目标汇编逐字节相同，6 个差异用例的五次 QEMU 动态指令配对 GM 为 1.005142、95% CI 下界为 1.000000、最差为 1.000000。板端六用例 wall-clock GM 为 1.054013，但 CI 下界为 0.993528；最差 0.943336 的两侧目标 `.text` 实际相同，属于毫秒级进程测量噪声。原始样本和三次被 fail-fast 废弃的诊断批次均保留在测量归档中。该结论关闭当前非 BOOM 的代码退化风险，但不通过 BOOM/LLVM 发布门。
