...
`profile.id` 是稳定标识；只有现场原始样本通过质量门槛后，`profilec` 才生成 `calibrated: true`。手填或声明型开发数据必须为 `false`。

`target` 声明 ISA、ABI、code model、频率以及 fetch/issue/retire 宽度。`measurement_environment` 记录实际后端、`rdcycle`/`rdinstret` 可用性和最终计时源；未校准开发 Profile 使用 `unmeasured` 与 `null`，校准 Profile 不允许保留未测值。R1 正式默认是 `rv64gc`、`lp64d`、`medany`、双发射 BOOM v3。SIMD 注册表和 ABI 字段保留，但正式 ISA/ABI 和许可未发布前必须 `enabled: false`，不得填造成本。
...
校准归档必须恰好覆盖注册表中的全部指标，每项恰好九个正整数原始样本，并与 `measurement_environment.timer` 的单位一致。上三角 pairing 每项只测一次，由 `profilec` 镜像成完整对称矩阵；缺项、重复项或额外项都不会继承开发 Profile 的占位值。
