...
final class InlineFunction {
  ...
  static void inline(Instruction call) {
    ...
    Function caller = callBlock.getParent();
    int id = nextId++;

    BasicBlock continuation =
        caller.insertBlockAfter(callBlock, callBlock.getLabel() + ".inline.cont." + id);
    moveTail(call, callBlock, continuation);
    retargetSuccessorPhis(callBlock, continuation);
    ...
    BasicBlock insertionPoint = callBlock;
    for (BasicBlock source : callee.getBlocks()) {
      BasicBlock clone =
          caller.insertBlockAfter(insertionPoint, source.getLabel() + ".inline." + id);
    ...
    new IRBuilder(callBlock).createBr((BasicBlock) values.get(callee.getEntryBlock()));
