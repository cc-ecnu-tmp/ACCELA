...
[ "$#" -eq 3 ] || fail "usage: scripts/benchmark-qemu-hotblocks.sh PROGRAM_ELF METRIC_LOG INPUT_FILE"
...
hotblocks=${QEMU_HOTBLOCK_PLUGIN:-$plugin_dir/hotblocks.$suffix}
...
[ -f "$hotblocks" ] || fail "hotblocks plugin is missing; run scripts/build-qemu-plugins.sh"
...
exec "$qemu" \
  ...
  -plugin "$profile" -plugin "$cache" -plugin "$hotblocks" \
  -d plugin -D "$metric_log" \
  </dev/null
